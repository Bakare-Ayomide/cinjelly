...
pre_return = text[:ret_idx]

def get_snippet(start_marker, end_marker):
    s = text.find(start_marker)
    if s == -1: raise Exception(f"Missing start: {start_marker}")
    e = text.find(end_marker, s)
    if e == -1: raise Exception(f"Missing end: {end_marker}")
    return text[s:e].strip()

email_banner = get_snippet('{/* Email Verification Banner */}', '{/* Welcome Header */}')
welcome_header = get_snippet('{/* Welcome Header */}', '{/* Quick Action Bento Grid */}')
player_card = get_snippet('{/* Player Launch Card */}', '{/* Side Membership Details Card */}')

# Broadcast feed
bc_feed = get_snippet('{/* Targeted Broadcast Notifications Feed */}', '{/* Dynamic Card Area */}')

# Dynamic cards (payment options & awaiting status)
dyn_cards = get_snippet('{/* Dynamic Card Area */}', '{/* Direct Debit Status Widget */}')

# Media requests console
media_reqs = get_snippet('{/* User Movie/Show Request Console */}', '{/* Affiliate Referral Program Section */}')

# Affiliate section
aff_sec = get_snippet('{/* Affiliate Referral Program Section */}', '{/* Mobile App Download Advertisement Banner */}')

# Mobile app download section
mob_app = get_snippet('{/* Mobile App Download Advertisement Banner */}', '{/* Support & Contact Details Section */}')

# Support section
sup_sec = get_snippet('{/* Support & Contact Details Section */}', '{/* ================= TAB 8: ACCOUNT & SECURITY ================= */}')
# clean up any trailing closing tags from prior run
if sup_sec.endswith('</main>'):
    sup_sec = sup_sec[:sup_sec.find('</main>')].strip()
...
# Modals
modals_start = text.find('{/* RE-SYNC SESSION CREDENTIALS MODAL */}')
modals_raw = text[modals_start:]
# strip any broken trailing container closings
modals_clean = modals_raw[:modals_raw.rfind('</div>')]
...
print("UserPortal.tsx completely regenerated!")
