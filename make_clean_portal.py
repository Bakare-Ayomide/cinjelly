...
# Read original full components/UserPortal.tsx if we can extract the base parts cleanly
with open('src/components/UserPortal.tsx', 'r') as f:
    raw = f.read()

# Let's inspect the original snippets by looking for core inner blocks:
def find_block(start_str, end_str):
    s = raw.find(start_str)
    if s == -1: raise Exception(f"Start not found: {start_str}")
    e = raw.find(end_str, s)
    if e == -1: raise Exception(f"End not found: {end_str}")
    return raw[s:e].strip()

# 1. Pre-return
pre_return = raw[:raw.find('  return (\n    <div className="min-h-screen')]

# Snippet 1: Email Banner
email_banner = find_block('{/* Email Verification Banner */}', '{/* Welcome Header */}')

# Snippet 2: Welcome Header
welcome_header = find_block('{/* Welcome Header */}', '{/* Quick Action Bento Grid */}')

# Snippet 3: Player Launch Card
player_card = find_block('{/* Player Launch Card */}', '{/* Side Membership Details Card */}')

# Snippet 4: Broadcast Feed
bc_feed = find_block('{/* Targeted Broadcast Notifications Feed */}', '{/* ================= TAB 2: SUBSCRIPTION')
# strip any trailing closing tags from previous run
if bc_feed.endswith('</div>\n        )}'):
    bc_feed = bc_feed[:-len('</div>\n        )}')].strip()

# Snippet 5: Dynamic Cards (Payment & Active Stream Status)
# starts at {!isActive ? ( or {/* Dynamic Card Area */}
dyn_cards_start = raw.find('{/* Dynamic Card Area */}')
dyn_cards_end = raw.find('{/* ================= TAB 3: DIRECT DEBIT')
dyn_cards = raw[dyn_cards_start:dyn_cards_end].strip()
if dyn_cards.endswith('</div>\n        )}'):
    dyn_cards = dyn_cards[:-len('</div>\n        )}')].strip()

# Snippet 6: Media requests card
req_start = raw.find('{/* User Movie/Show Request Console */}')
req_end = raw.find('{/* Custom Movie/Show Request Modal Form */}')
req_card = raw[req_start:req_end].strip()

# Custom Request Modal Form
req_modal = find_block('{/* Custom Movie/Show Request Modal Form */}', '{/* ================= TAB 5: AFFILIATE')
if req_modal.endswith('</div>\n        )}'):
    req_modal = req_modal[:-len('</div>\n        )}')].strip()

# Snippet 7: Affiliate section
aff_start = raw.find('{/* Affiliate Referral Program Section */}')
aff_end = raw.find('{/* ================= TAB 6: MOBILE')
aff_sec = raw[aff_start:aff_end].strip()
if aff_sec.endswith('</div>\n        )}'):
    aff_sec = aff_sec[:-len('</div>\n        )}')].strip()

# Snippet 8: Mobile App section
mob_start = raw.find('{/* Mobile App Download Advertisement Banner */}')
mob_end = raw.find('{/* ================= TAB 7: SUPPORT')
mob_sec = raw[mob_start:mob_end].strip()
if mob_sec.endswith('</div>\n        )}'):
    mob_sec = mob_sec[:-len('</div>\n        )}')].strip()

# Snippet 9: Support section
sup_start = raw.find('{/* Support & Contact Details Section */}')
sup_end = raw.find('{/* ================= TAB 8: ACCOUNT')
sup_sec = raw[sup_start:sup_end].strip()
if sup_sec.endswith('</div>\n        )}'):
    sup_sec = sup_sec[:-len('</div>\n        )}')].strip()

# Modals
modals_start = raw.find('{/* RE-SYNC SESSION CREDENTIALS MODAL */}')
modals_end = raw.rfind('</div>\n  );\n}')
modals_sec = raw[modals_start:modals_end].strip()

side_card = """            {/* Side Membership Details Card */}
...
direct_debit_screen = """            <div className="bg-[#120507] border border-[#2e1015] rounded-2xl p-6 sm:p-8 shadow-xl relative overflow-hidden">
...
print("make_clean_portal completed!")
