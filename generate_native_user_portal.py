...
email_banner = find_section('{/* Email Verification Banner */}', '{/* Welcome Header */}')
welcome_header = find_section('{/* Welcome Header */}', '{/* Dynamic Card Area */}')
dynamic_cards = find_section('{/* Dynamic Card Area */}', '{/* Player Launch Card */}')
player_card = find_section('{/* Player Launch Card */}', '{/* Side Membership Details Card */}')
membership_card = find_section('{/* Side Membership Details Card */}', '{/* Direct Debit Status Widget */}')
direct_debit_widget = find_section('{/* Direct Debit Status Widget */}', '{/* Mobile App Download Advertisement Banner */}')
mobile_app_banner = find_section('{/* Mobile App Download Advertisement Banner */}', '{/* Community Announcements Feed & Media Requests Section */}')

broadcast_feed = find_section('{/* Targeted Broadcast Notifications Feed */}', '{/* User Movie/Show Request Console */}')
media_requests_sec = find_section('{/* User Movie/Show Request Console */}', '{/* Affiliate Referral Program Section */}')

affiliate_sec = find_section('{/* Affiliate Referral Program Section */}', '{/* Support & Contact Details Section */}')
support_sec = find_section('{/* Support & Contact Details Section */}', '{/* RE-SYNC SESSION CREDENTIALS MODAL */}')

modals_sec = text[text.find('{/* RE-SYNC SESSION CREDENTIALS MODAL */}'):]
# trim trailing closing tag for the old container
last_closing = modals_sec.rfind('</div>')
modals_clean = modals_sec[:last_closing]
...
final_code = pre_return + new_return

with open('src/components/UserPortal.tsx', 'w') as f:
    f.write(final_code)

print("UserPortal.tsx successfully refactored to Native Mobile App layout!")
